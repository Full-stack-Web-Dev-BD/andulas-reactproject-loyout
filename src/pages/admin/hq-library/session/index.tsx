...
const HQLibrarySession = () => {
        ...
        filters: JSON.stringify([
          Object.fromEntries(
            Object.entries({
              ...
              topicType: TopicType.HQ_LIBRARY,
  ...
  const { mutate: mutateDuplicateSession } = useMutation('duplicateSession', duplicateSession, {
    onSuccess: (data) => {
      ...
      setMessageDuplicateSessionSuccess(
        `${sessionDuplicate?.sessionName} from ${sessionDuplicate?.topicName + '-' + sessionDuplicate?.moduleName
        } has been successfully duplicated to ${topicNameDuplicate?.label + '-' + moduleNameDuplicate?.label
        }`,
      );
    },
    onError: ({ response }) => {
  ...
  const redirectSessionDetail = (id: number, statusRedirect: string) => {
    history(
      `${ROUTES.hq_library_session}/${id}${statusRedirect === Status.COMPLETED ? '/overview' : ''}`,
    );
  };
    ...
    (value: string) => {
      if (value === 'selection') {
        ...
        setMessageConfirmDelete(
          `Are you sure you want to delete the selected ${selection?.length > 1 ? 'sessions' : 'session'
  ...
  const handleReset = useCallback(() => {
    setOrder('ASC');
    setLimit('5');
    setFilters({ topicType: TopicType.HQ_LIBRARY });
    setSearchValue('');
    setStatus('');
  }, [limit]);
  ...
  const handleSearchModuleOfTopic = (id: string | number) => {
    if (id) {
      setFilterModules(JSON.stringify([{ topicID: Number(id), topicType: TopicType.HQ_LIBRARY }]));
      getModules({
        ...PARAMS_SELECT_SEARCH.module,
        filters: JSON.stringify([{ topicID: Number(id), topicType: TopicType.HQ_LIBRARY }]),
      });
      return;
    }
    setTimeout(() => {
