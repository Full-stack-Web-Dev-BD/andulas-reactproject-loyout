...
const ModuleSession = () => {
  ...
  const redirectSessionDetail = (id: number, statusRedirect: string) => {
    if (
      ...
      history(
        `${ROUTES.hq_library_topic}/${topicId}/module/${moduleId}/session/${id}${
          statusRedirect === Status.COMPLETED ? '/overview' : ''
        }`,
      );
    } else {
      history(
        `${ROUTES.hq_library_module}/${moduleId}/session/${id}${
          statusRedirect === Status.COMPLETED ? '/overview' : ''
        }`,
      );
    }
  ...
  return (
    <>
      <SessionList
        ...
        isFilter={!!searchValue || Object.values(filters).filter(x => x).length > 1}
      />
