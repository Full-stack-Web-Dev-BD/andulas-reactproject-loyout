...
const HQLibrarySessionTeacher = () => {
    ...
    onSuccess: ({
      ...
      const sessions = data.records.map((el) => {
        return {
          ...
          moduleName: el?.module?.moduleName,
  ...
  const redirectSessionDetail = (id: number, statusRedirect: string) => {
    history(
      `${ROUTES.hq_library}/topic/${topicId}/module/${moduleId}/session/${id}/overview/teacher`,
    );
  };
    ...
    (value: string) => {
      ...
      timeout.current = setTimeout(() => {
        searchListSessions({
          ...
          order: 'ASC',
          sort,
          filters: JSON.stringify([{ moduleID: Number(moduleId), topicType: TopicType.HQ_LIBRARY }]),
        });
      }, 500);
    },
...
export default HQLibrarySessionTeacher;
