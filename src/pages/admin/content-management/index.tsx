...
import ModalDuplicateSession from './session/duplicate-session';
...
const HQLibrarySession = () => {
  ...
  const { mutate: searchListSessions, isLoading: isLoadingSearchSessions } = useMutation('searchSessions', searchSessions, {
  ...
  const { mutate: getListSessions, isLoading: isLoadingGetSessions } = useMutation('searchSessions', searchSessions, {
    onSuccess: ({
      data,
    }: {
        ...
        filters: JSON.stringify([
          Object.fromEntries(
            Object.entries({
              ...
              topicType: [RoleName.SUPER_ADMIN, RoleName.ADMIN].includes(roleName?.trim()) ? TopicType.HQ_LIBRARY : TopicType.COMMUNITY_LIBRARY,
            }).filter(([, v]) => (v as string)?.toString() !== ''),
          ),
        ]),
      });
    },
    [limit, pagination.current, searchValue, renderOrder, renderSort, filters, status, roleName],
  );
  ...
  const { mutate: mutateDeleteSession } = useMutation('deleteSession', deleteSession, {
    ...
    onError: ({ response }: { response: { data: { message: string }; status: number } }) => {
      setIsModalDeleteSession(false);
      if(response.status == 403) {
        setMessageWarning("You are not allowed to delete content management.")
      } else {
  ...
  useEffect(() => {
    handleGetListSessions();
  }, [limit, pagination.current, searchValue, sort, order, filters, status, roleName]);
  ...
  useEffect(() => {
    ...
    if ([RoleName.SUPER_ADMIN, RoleName.ADMIN].includes(roleName?.trim())) {
      getTopics({
        ...
        filters: JSON.stringify([
          Object.fromEntries(
            Object.entries({ topicType: TopicType.HQ_LIBRARY }).filter(
      ...
      getTopics({
        ...
        filters: JSON.stringify([
          Object.fromEntries(
            Object.entries({ topicType: TopicType.COMMUNITY_LIBRARY }).filter(
              ([, v]) => (v as string)?.toString() !== '',
            ),
          ),
        ]),
  ...
  const redirectSessionDetail = (id: number, statusRedirect: string) => {
    history(
      `${ROUTES.content_management_update_session}/${id}`,
    );
  };
  ...
  const onFinish = useCallback(
    ...
    [limit, pagination, order, roleName],
  );
  ...
  const debounceSearch = useCallback(
    ...
    [limit, pagination.current, timeout, sort, roleName],
  );
  ...
  const handleSearchModules = useCallback(
    (value: string) => {
      clearTimeout(timeout?.current);
      timeout.current = setTimeout(() => {
        getModules({ ...PARAMS_SELECT_SEARCH.module, search: value, filters: filterModules });
        return;
      }, 50);
    },
  ...
  const handleReset = useCallback(() => {
    setOrder('ASC');
    setLimit('5');
    setFilters({ topicType: [RoleName.SUPER_ADMIN, RoleName.ADMIN].includes(roleName?.trim()) ? TopicType.HQ_LIBRARY : TopicType.COMMUNITY_LIBRARY });
    setSearchValue('');
    setStatus('');
  }, [limit, roleName]);
  ...
  const handleSearchModuleOfTopic = (id: string | number) => {
    if (id) {
      setFilterModules(JSON.stringify([{ topicID: Number(id), topicType: [RoleName.SUPER_ADMIN, RoleName.ADMIN].includes(roleName?.trim()) ? TopicType.HQ_LIBRARY : TopicType.COMMUNITY_LIBRARY }]));
      getModules({
        ...PARAMS_SELECT_SEARCH.module,
        filters: JSON.stringify([{ topicID: Number(id), topicType: [RoleName.SUPER_ADMIN, RoleName.ADMIN].includes(roleName?.trim()) ? TopicType.HQ_LIBRARY : TopicType.COMMUNITY_LIBRARY }]),
      });
      return;
    }
    setTimeout(() => {
      getModules({
        ...PARAMS_SELECT_SEARCH.module,
        filters: JSON.stringify([{ topicType: [RoleName.SUPER_ADMIN, RoleName.ADMIN].includes(roleName?.trim()) ? TopicType.HQ_LIBRARY : TopicType.COMMUNITY_LIBRARY }]),
      });
    }, 100);
    setFilterModules('');
  };
  ...
  return (
    <>
      <SessionList
        ...
        isLoading={isLoadingGetSessions}
      />
...
export default HQLibrarySession;
