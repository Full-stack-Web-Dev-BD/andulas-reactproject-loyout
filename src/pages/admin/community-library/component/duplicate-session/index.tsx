...
interface IProps {
  ...
  isKeepOpen: boolean;
  handleDuplicateSession: (values: { topicName: IOptionItem; moduleName: IOptionItem }) => void;
...
const ModalDuplicateSession = (props: IProps) => {
  ...
  const [modulesOfTopicOptions, setModulesOfTopicOptions] = useState<
    { label: string; value: string; isDisabled?: boolean }[]
  >([]);
  const [isOpenModalConfirmDuplicateSession, setIsOpenModalConfirmDuplicateSession] =
    useState(false);
  ...
  useEffect(() => {
    if (topicName) {
      getModulesOfTopic({
        ...
        filters: JSON.stringify([{ topicID: Number(topicName?.value) }]),
      });
    }
  }, [topicName]);
  ...
  return (
    <>
      ...
      >
        <Form form={formDuplicateSession} layout="vertical" colon={true} onFinish={handleSubmit}>
          <Form.Item
