import { Form, Layout, notification, TablePaginationConfig } from 'antd';
...
import ModalCreateNewTopic from './new-topic';
...
const CommunityLibraryTopicTeacher = () => {
  ...
  const { mutate: handleCreateNewTopic } = useMutation('createNewTopic', createNewTopic, {
    onSuccess: () => {
      handleGetListTopics();
      setIsOpenModalCreateNewTopic(false);
      formCreateNewTopic.resetFields();
    },
    onError: ({ response }) => {
      if (response.status == 403) {
        notification.error({ message: 'You are not allowed to create topic.' });
      } else {
        formCreateNewTopic.setFields([{ name: 'topicName', errors: [response.data.message] }]);
      }
      setIsKeepOpen(true);
    },
  });

  const { mutate: handleEditTopic } = useMutation('editTopic', editTopic, {
    onSuccess: () => {
      handleGetListTopics();
      setIsOpenModalCreateNewTopic(false);
      formCreateNewTopic.resetFields();
    ...
    onError: ({ response }) => {
      if (response.status == 403) {
        notification.error({ message: 'You are not allowed to edit topic.' });
      } else {
        formCreateNewTopic.setFields([{ name: 'topicName', errors: [response.data.message] }]);
      }
      setIsKeepOpen(true);
  ...
  const { mutate: mutateDeleteTopic } = useMutation('deleteTopic', deleteTopic, {
    ...
    onError: ({ response }: { response: { data: { message: string }; status: number } }) => {
      if (response.status == 403) {
        setMessageWarning('You are not allowed to delete topic.');
      } else {
  ...
  const { mutate: mutateDeleteMultipleTopics } = useMutation(
    ...
    deleteMultipleTopics,
    {
      ...
      onError: ({ response }) => {
        if (response.status == 403) {
          setMessageWarning('You are not allowed to delete topics.');
        } else {
          setMessageWarning(response.data.message);
        }
      },
  ...
  return (
    <Layout className="bg-transparent flex flex-col gap-y-6">
      <div className="flex justify-between items-center bg-transparent px-0 ">
        ...
        <ButtonCustom onClick={() => setIsOpenModalCreateNewTopic(true)} color="orange">
          New Topic
        </ButtonCustom>
      </div>
      ...
      <FilterCard
        ...
        pathSearchDetail={ROUTES.community_library_topic}
        pathEndPointSearchDetail="/module/teacher"
        keyResult="topicName"
      />

      <CardResult
        onRedirect={(id) => {
          history(`${ROUTES.community_library_topic}/${id}/module/teacher`);
        }}
        setMessageWarning={setMessageWarning}
        handleOpenModalEdit={handleOpenModalEditTopic}
        ...
        action={{
          show: selectedTopics.length > 0 ? true : false,
      ...
      {isOpenModalCreateNewTopic && (
        <ModalCreateNewTopic
          isKeepOpen={isKeepOpen}
          topic={topic as ICardItem}
          form={formCreateNewTopic}
          visible={true}
          onCancel={() => {
            setIsOpenModalCreateNewTopic(false);
            formCreateNewTopic.resetFields();
...
export default CommunityLibraryTopicTeacher;
